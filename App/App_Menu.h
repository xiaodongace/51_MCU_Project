...
/* 掌机模式的游戏个数（贪吃蛇 / 打砖块 / 飞机大战）。
 * 放在头文件里是因为**显示层也要按它循环画列表** ——
 * 尺寸/数量类的宏只允许定义一处（规范：改图幅/数量必须两边一起改）。 */
#define GAME_COUNT          3
...
u8  Menu_DateErr(void);         /* 上一次提交是否因非法被拒（1=非法）*/

/* 设置页的两个状态（第 7 点）*/
u8 Menu_SetIsEditing(void);
u8 Menu_SetSel(void);

/* 「测距仪」任务：最近一次测到的距离（厘米）。0 = 没测到 / 超出量程。
 * 显示层读它来画数值和进度条。 */
...
/*========================================================================
 *  掌机模式（"7 游戏"任务）—— 显示层要用到的两个状态查询
 *
 *  这一页有**两个状态**（和闹钟/番茄钟同款，不额外压栈）：
 *      列表态：SPI 列出 3 个游戏，K1/K3 选、K2 开始、K4 回任务清单
 *      游玩态：SPI 是游戏画面（游戏自己刷屏），I2C 副屏关掉
 *  （另有一个内部"结算态"，只停 1.5 秒显示 GameOver，显示层当游玩态处理）
 *========================================================================*/
