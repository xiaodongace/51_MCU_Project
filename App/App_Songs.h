...
/* 基本时值。只改这四个数就能整体调快慢（音符表里全是引用它们，不用动曲子）。
 *
 * 【第 70 轮订正】原来这段注释写的是"现在整体减半：BASE_Q 400 -> 200，
 *   BASE_E 200 -> 100，BASE_QD 600 -> 300，BASE_H 800 -> 400"，
 *   但**代码里的实际值是 300 / 100 / 300 / 400** —— BASE_Q 是 300 不是 200，
 *   注释与代码不符（注释还自己写着"BASE_Q = 200ms 相当于 300 拍"，对不上）。
 *   这里按代码的实际值改写。
 *
 * 【一处要留意的乐理问题（未改，因为改它会改声音）】
 *   BASE_QD（附点四分）现在**等于** BASE_Q（四分）= 300ms，
 *   而附点四分音符按定义应是四分的 1.5 倍（450ms）。
 *   现状听起来的实际效果是"该拖长的地方没拖长"。
 *   要修就改 BASE_QD，但**必须先听一遍再定**，别凭空改。 */
#define BASE_Q      300         /* 四分音符 */
#define BASE_E      100         /* 八分音符 */
#define BASE_QD     300         /* 附点四分音符（= 四分，见上面的说明） */
