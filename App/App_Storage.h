 ...
 *
 *   0 号区 0xF000  设置 Settings_t + 8 组闹钟 AlarmItem_t[8]
 ...
 *     完整数据再擦，避免擦完写失败导致全丢。"
 *   —— 设置和 8 组闹钟放在同一区，一次擦除、一次写回，中间不会出现"擦了没写"的窗口。
 *
 * 记录为什么不用每次都回写 0 号区：
...
/* 上电装载：读设置与 8 组闹钟。数据无效时填入默认值（返回 ST_ERR_MAGIC）。 */
u8 Storage_Load(void);

/* 保存设置 + 8 组闹钟（一次擦除 + 一次写回，原子性最好） */
