...
void I2C_Lock(void)
{
    ...
    while (!I2C_TryLock())
    {
        ...
        /* 【2026-09-18 修正】原来这里在 500ms 处会 printf 一行，真机日志显示
         * 它会在若干秒内反复触发 —— 把 printf 放进了热路径，而 printf 是 C51 里
         * 栈开销最大的函数（链接映射里 FREE_STACK 只剩 20 字节）。
         * 【第 70 轮】那句"4 秒时打印一次"的 printf 后来也删了，现在这里**没有任何
         * 打印**：超时就静默强夺锁。这条路本来就"不该发生"，安静处理即可。 */
