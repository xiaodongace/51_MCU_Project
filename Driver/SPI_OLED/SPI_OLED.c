...
/*========================================================================
 *  显存版 ASCII 文字 —— 给"显存通路"用（游戏结算画面等）
 *
 *  为什么需要这两个函数：
 *    驱动原本只有**直写屏**版的 Display_8x16 / Display_GB2312_string，
 *    而游戏走的是**显存通路**（GClear -> DrawPoint -> Refresh）。
 *    结算画面当初直接调了直写屏版，结果被紧接着的 Refresh 用空显存覆盖掉，
 *    画面全黑（2026-09-23 真机现象）。补上显存版后两条通路各自完整。
 *    约定见 spi_oled.h 顶部。
 *========================================================================*/

/*------------------------------------------------------------------------
 *  把一个 8x16 点阵写进显存（不碰屏，等 SPI_OLED_Refresh 一起送）
 *
 *  x    起始列（0..127）
 *  page 起始页（0..7）；一个 8x16 字占 page 和 page+1 两页
 *  dp   16 字节点阵：dp[0..7] 上半页、dp[8..15] 下半页
 *       （与 SPI_OLED_Display_8x16 的取点顺序一致，可以直接复用它的字模）
 *
 *  ※ 边界必须夹住：显存只有 1024 字节，而且它在 xdata 区、
 *    越界会踩到别的变量，现象会很离奇且难查。
 *------------------------------------------------------------------------*/
void SPI_OLED_GBuf_8x16(u8 x, u8 page, u8 *dp)
{
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            SPI_OLED_GRAM[idx + i] = (char)dp[j * 8 + i];
        }
    }
}

/*------------------------------------------------------------------------
 *  在显存上画一串 ASCII（点阵从屏上的晶联讯字库 IC 读）
 *
 *  地址公式与直写屏版 GB2312_string 的 ASCII 分支**完全相同**：
 *      Address = 0x3CF80 + (ASCII - 0x20) * 16
 *
 *  ※ 只支持 ASCII（0x20..0x7E）。汉字是 16x16 点阵、要占两倍宽度，
 *    本函数不处理 —— 菜单的汉字走直写屏通路，不需要这个。
 *------------------------------------------------------------------------*/
void SPI_OLED_GBuf_string(u8 x, u8 page, u8 *text)
{
    ...
    while (text[i] != 0x00)
    {
        if ((text[i] >= 0x20) && (text[i] <= 0x7e))
        {
            ...
            addr = 0x3CF80UL + (u32)(text[i] - 0x20) * 16UL;
            SPI_OLED_get_data_from_ROM((u8)(addr >> 16),
                                       (u8)(addr >> 8),
                                       (u8)(addr & 0xFF),
                                       buf, 16);
            SPI_OLED_GBuf_8x16(x, page, buf);
