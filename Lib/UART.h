...
/*  【2026-09-22 本项目裁剪】本工程只用串口 1：P3.0 RxD / P3.1 TxD 经板载 CH340N 到 USB，
 *  对接 Python 上位机设置闹钟。UART2/3/4 全工程无任何引用（App/Driver/User 层零出现），
 *  因此按 STC 官方注释的建议把它们关掉。
 *
 *  关闭后自动生效的三处收益（都是 #ifdef 保护，不需要改别的代码）：
 *    1) Lib/UART.c 里 UART2/3/4 的配置分支不再编译（每个分支几十行）；
 *    2) TX2/TX3/TX4/RX2/RX3/RX4 六个缓冲不再分配
 *       （128+128 + 64+64 + 32+32 = 448 字节 xdata）；
 *    3) Lib/UART_Isr.c 里 UART2/3/4 的中断服务函数不再编译（也不会生成中断向量）。
 *
 *  以后若真要用串口 2/3/4：把下面 comment 掉的三行恢复即可，其余代码无需改动。 */
//#define	UART2	2
//#define	UART3	3
//#define	UART4	4
